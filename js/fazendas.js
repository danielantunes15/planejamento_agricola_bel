...
async function loadFarms() {
    ...
    const { data, error } = await sb.from('fazendas').select('*');
...
async function saveFarmDB() {
    toggleLoader(true);
    const payload = {
        cod_fazenda: elsMap.inputCod.value,
        name: elsMap.inputName.value,
        owner: elsMap.inputOwner.value,
        area_ha: parseFloat(elsMap.modalSummary.innerText.replace('Total: ','').replace(' ha','')),
        geojson: { type: "FeatureCollection", features: currentFeaturesData.map(i=>i.feature) }
    };
    let err;
    if(editingFarmId) { 
        err = (await sb.from('fazendas').update(payload).eq('id', editingFarmId)).error; 
    } else { 
        err = (await sb.from('fazendas').insert(payload)).error; 
    }
